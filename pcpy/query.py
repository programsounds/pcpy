...
This module includes functions that collectively generate a JSON file for
queries related to pcsets and modal set complexes.

A fresh copy of the JSON file can be made through directly running this
module script--a new catalog.json will be created in the same directory
where this program is in.

This module decodes the JSON file and assign the data to the variable,
catalog. So the queries can be made through importing the variable in other
scripts.

The variable, catalog, is a dict with nested dict structures. The nested
dicts are associated with four top-level keys, PFToSN, SC, inclusionTable,
and MSC. Each key corresponds with the subject of query. Note that it is
always string data type for the keys of the catalog dict.
...
SC (key=SN, value={PF, ICV, Z-corr, symmetry, MA, MSC})
    ...
    A mapping for SCs and their profiles. The top-level value is a nested dict.
    ...
    Z-corr: a str for Z-correspondent represented by a str of SN if any,
        None otherwise
    symmetry: a list of two ints for the degrees of transpositional and
        inversional symmetries
    MA: a list with string elements for modal attribute symbols
    MSC: a list of MSC nexus sets, represented by SNs, to which the SC is a
        member.

    The top-level query is made by SN. If the current data structure of a
    set is an iterable of prime form, use toPFStr() and PFToSN to obtain the
    SN of the set.

inclusionTable (key=SN, value={{n: [inclVec]}, ..., {n+i: [inclVec]})

    A mapping for inclusion tables of SCs. The top-level value is a nested
    dict that represents the inclusion table of the SC. Each inclusion table
    is also a dict that is an inclusion vector of a given cardinality.

    SN: a str for set name
    n, ..., n+i: ints for the cardinalities between 3 and 9 except for that
        of the query target
    inclVec: a list with ints representing the inclusion vector

    For a set-class S of cardinality j and inclusion vector of cardinality
    k, if j < k, the vector gives the number of abstractly included set
    classes; if j > k, the vector gives the number of distinct members of
    set-classes that include S.

    Note that it is important to distinguish the covering (j > k) of S by a
    set-class Q from the embedding (j < k) of S in Q: the numbers are not
    always the same. In some cases, counts for covering is smaller than
    those of embedding. This is due to the elimination of Tn/TnI equivalent
    sets in the case of covering. Therefore, to find how many instances of
    S are embedded in Q, one looks at the entry for Q; if one wants to know
    the number of members of Q that cover S, one consults the entry for S.
...
MSC ({key=SN, value={card: [{member1}...{memberN}]})
    ...
    represented by a dict {SN, MA, symmetry, inclusion, Z-corr}:
...
def mapPFToSN(pfs):
    ...
    :param pfs: a sorted list of pcsets which correspond with all the PFs of
...
def countInclusions(s1, s2):
    """
    A function to compute the number of instances s1 includes s2 and vice
    versa: given that SC1 and SC2 are the set-classes of s1 and s2
    respectively, if s1 is larger than s2, the function returns the number
    of instances of each included SC2 in SC1, namely the number of abstract
    subsets of SC2 in SC1; whereas, if s1 is smaller than s2, the function
    returns the number of unique members of SC2 that include SC1--"unique"
    denotes elimination of duplicates from equivalent Tn and TnI members of
    SC2.
